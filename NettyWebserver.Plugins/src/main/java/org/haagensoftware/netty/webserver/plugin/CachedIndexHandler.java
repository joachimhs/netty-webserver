...
import org.haagensoftware.netty.webserver.scriptCache.ScriptCache;
import org.haagensoftware.netty.webserver.scriptCache.ScriptHash;
...
public class CachedIndexHandler extends FileServerHandler {
	...
	public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) throws Exception {
        ...
        if (cache == null || cache.isExpired()) {
        	...
        	logger.info("Updating index.html from filesystem");
	...
	private ScriptCache updateScriptCacheForPath(String path) throws IOException {
		...
		if (htmlDocument != null) {
			...
			for (Element scriptElement : scriptElements) {
				...
				if (scriptSrc != null && scriptSrc.endsWith(".js")) {
		...
		ScriptCache cache = ScriptHash.updateScriptContents(path, scriptPathList, scriptContentsCombined.toString(), htmlDocument.html(), System.currentTimeMillis() + (maxCacheSeconds * 1000));
